    ...
    class AmazonScrapper < Scrapper
          ...
          li.each_with_index do |li_item, index|
            ...
            unless meta.empty? || meta[:isbn].nil?
              li_map = { "#{meta[:isbn]}" => {
                ...
                description: meta[:description],
                book_meta_data: { "#{site_id[:id]}" => { rating: meta[:rating], price: meta[:price], rating_count: meta[:rating_count],
      ...
      def self.process_sub_page(sub_page)
        ...
        unless sub_page.nil?
          ...
          details.merge!('title'.to_sym => sub_page.search('#btAsinTitle').text().squish.strip.gsub(" [Paperback]","").gsub(" [Hardcover]",""))
          begin
